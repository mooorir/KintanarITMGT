...
def caesar_cipher(message, shift):
    ...
    for letter in message:
        if letter == " ":
            ...
            processed_message.append(chr(((ord(letter) - ord("A")) + shift) % 26 + ord("A")))
...
def shift_by_letter(letter, letter_shift):
    ...
    if letter == " ":
        ...
        translated_shift = (ord(letter_shift) - ord("A")) % 26
        return chr((((ord(letter) - ord("A")) + translated_shift)) % 26 + ord("A"))
...
def vigenere_cipher(message, key):
    ...
    for x, char in enumerate(message):
        if char == " ":
            cypher_answer.append(" ")
        elif "A" <= char <= "Z":
            specific_shift = ord(key[x % len(key)]) - ord("A")
            shifted_char = chr((((ord(char)) - ord("A") + specific_shift) % 26) + ord("A"))
            cypher_answer.append(shifted_char)
    return "".join(cypher_answer)
